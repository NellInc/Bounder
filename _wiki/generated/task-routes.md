...
| `public_copy` | `*.html`<br>`styles.css`<br>`site.js`<br>`README.md`<br>`SECURITY.md`<br>`guides/**`<br>`tests/interface.test.js` | `simulator`<br>`publication_builder` | `unit_coverage`<br>`build`<br>`browser`<br>`design_lint`<br>`docs_check` | `source_behavior`<br>`browser_behavior`<br>`publisher_integrity` | Yes |
