...
|---|---|---|---|---|---|
| `control_model` | `system/**`<br>`scripts/lib/system-model.mjs`<br>`scripts/system-inspect.mjs`<br>`scripts/check-changed.mjs`<br>`scripts/docs-check.mjs`<br>`scripts/verify.mjs`<br>`tests/system-model.test.js`<br>`tests/agent-commands.test.js`<br>`package.json`<br>`scripts/generate-system-views.mjs`<br>`scripts/verify-changed.mjs`<br>`.github/generated/impact-rules.json`<br>`_wiki/generated/task-routes.md`<br>`tests/browser-module-boundaries.test.js`<br>`tests/producer-derivation.test.js`<br>`.github/workflows/site-quality.yml`<br>`.gitignore`<br>`scripts/validate-system.mjs`<br>`tests/generated-control.test.js`<br>`.npmrc`<br>`.nvmrc`<br>`package-lock.json`<br>`tests/tooling-hygiene.test.js` | `agent_control_plane` | `system_check`<br>`unit`<br>`docs_check`<br>`verify`<br>`generate_system_views` | `source_behavior` | No |
...
| `publication` | `scripts/build-site.mjs`<br>`tests/publication.test.js`<br>`.github/workflows/deploy-pages.yml`<br>`playwright.config.js`<br>`tests/playwright-config.test.js` | `publication_builder` | `unit_coverage`<br>`build`<br>`browser` | `publisher_integrity`<br>`browser_behavior` | Yes |
| `release` | `scripts/generate-release-manifest.js`<br>`tests/release-manifest.test.js`<br>`release/**`<br>`VERSION`<br>`CHANGELOG.md`<br>`scripts/generate-release-manifest-v2.mjs`<br>`schemas/bounder-release-manifest-v2.schema.json`<br>`tests/release-manifest-v2.test.js`<br>`.github/workflows/receipt-drift.yml`<br>`scripts/lib/release-producer.mjs` | `release_manifest` | `unit_coverage`<br>`build`<br>`browser`<br>`docs_check`<br>`producer_derivation` | `publisher_integrity`<br>`producer_derivation`<br>`cross_repository_compatibility` | Yes |
| `public_copy` | `*.html`<br>`styles.css`<br>`site.js`<br>`README.md`<br>`SECURITY.md`<br>`guides/**`<br>`tests/interface.test.js`<br>`sitemap.xml`<br>`tests/page-security.test.js`<br>`tests/site-quality.test.js`<br>`tests/browser-entry-side-effects.test.js`<br>`tests/embedded-simulator-height.test.js` | `simulator`<br>`publication_builder` | `unit_coverage`<br>`build`<br>`browser`<br>`design_lint`<br>`docs_check` | `source_behavior`<br>`browser_behavior`<br>`publisher_integrity` | Yes |
...
| `repository_provenance` | `design/**`<br>`.github/ISSUE_TEMPLATE/**` | `architecture_knowledge` | `docs_check` | `source_behavior` | No |
