...
test("reference observability benchmark reports bounded payloads and one-pass aggregation without overclaiming", async () => {
  const ticks = [100, 125];
  const result = await runObservabilityBenchmark({ guardianCount: 1_000, clock: { now: () => ticks.shift() }, warmupRuns: 0, measuredRuns: 1 });
  assert.equal(result.passed, true);
  assert.equal(result.guardian_count, 1_000);
  assert.equal(result.aggregation_ms, 25);
  assert.deepEqual(result.aggregation_samples_ms, [25]);
...
test("benchmark enforces count and byte or time failures and its CLI emits stable JSON", async () => {
  ...
  const failed = await runObservabilityBenchmark({
    ...
    clock: { now: (() => { const values = [0, 3_000]; return () => values.shift(); })() },
    warmupRuns: 0,
    measuredRuns: 1
  });
  assert.equal(failed.passed, false);
  assert.match(failed.failures[0], /aggregation_ms/);
  const messages = [];
  const result = await runBenchmarkCli([], { log: (message) => messages.push(message) }, {
    guardianCount: 1_000,
    clock: { now: (() => { const values = [0, 25]; return () => values.shift(); })() },
    warmupRuns: 0,
    measuredRuns: 1
  });
  assert.equal(result.passed, true);
  assert.equal(JSON.parse(messages[0]).version, "bounder-observability-benchmark/v1");
