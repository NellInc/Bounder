...
| 2026-09-03 | Post-publication polish | An adversarially verified nine-dimension audit of the sealed v1.1.1 tree confirmed 64 findings, including checkpoint sequence 0 mis-aggregation, a weaker parser on the signed telemetry path, a publication lock with no staleness recovery, an unpinned tarball in the deploy gate, and an unstyled nested 404 | Fix every confirmed finding on the v1.1.2 line with regression tests, tighten three shared observability schemas rather than loosen the runtime, and pin the design linter in the lockfile | The three schema tightenings must land byte-identically in the private producer before v1.1.2 can seal; deployed behaviour remains unverified |
